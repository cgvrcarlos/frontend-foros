  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6">
        ...
        <span className="text-slate-900 font-medium">Respuestas de encuesta</span>
      </nav>

      <h1 className="text-2xl font-bold text-slate-900 mb-6">Respuestas de encuesta</h1>
      ...
      {data.length === 0 && !error ? (
        <div className="text-center py-16 bg-white rounded-xl border border-slate-200">
          <p className="text-slate-500">No hay respuestas registradas todavía.</p>
        </div>
      ) : (
        <div className="space-y-5">
          {data.map((g, i) => (
            <div key={i} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
              <h2 className="font-semibold text-slate-900 mb-1">{g.pregunta}</h2>
              ...
              <div className="space-y-2">
                {g.respuestas.map((r, j) => (
                  <div key={j} className="flex items-start gap-3 text-sm">
                    <span className="shrink-0 mt-0.5 w-5 h-5 rounded-full bg-slate-100 flex items-center justify-center text-xs text-slate-500 font-medium">
                      {j + 1}
                    </span>
                    <div className="flex-1">
                      ...
                      {r.usuario && (
                        <p className="text-xs text-slate-400 mt-0.5">{r.usuario}</p>
                      )}
                    </div>
                  </div>
                ))}
